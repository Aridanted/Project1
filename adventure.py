...
class AdventureGame:
    ...
    def take_command(self, taking_item_name: str) -> None:
        ...
        self.items_collected.add(itemm.name)
    ...
    def examine_command(self, item__name: str) -> None:
        ...
        USB Drive:
        BLUE USB drive with U of T sticker. The BLUE GUARDIAN. Contains your project code,
        but your partner mentioned a bug...
        """
...
if __name__ == "__main__":
    ...
    while game.ongoing:
        ...
        if location.id_num == 7 and not game.server_room_unlocked:
            commands["enter code"] = "action:keypad"
        if game_log.is_empty():
            game_log.add_event(Event(location.id_num, location.long_description))
        else:
            # choice is the command that led from the previous event to THIS one
        ...
        game.print_location_items(location)
