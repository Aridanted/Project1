...
class Event:
    ...
    Representation Invariants:
        - self.id_num >= 0
        - self.description != ''
        - self.next is None == self.next_command is None
    """
    id_num: int
    description: str
...
class EventList:
    ...
    Representation Invariants:
        ...
        - self.last.next_command is None
    ...
    def display_events(self) -> None:
        ...
        while curr is not None:
            if curr.next_command is None:
                ...
                print(
                    f"{event_num}. Location {curr.id_num}: {curr.description[:50]}... -> Command: {curr.next_command}")
            curr = curr.next
            event_num += 1
        print("=================\n")
    ...
    def add_event(self, event: Event, command: str = None) -> None:
        ...
        1
        """
        if self.is_empty():     # This is the first event
            self.first = event
            self.last = event
            event.prev = None
            event.next = None
    ...
    def remove_last_event(self) -> None:
        ...
        if self.is_empty():
            return
        elif self.last is self.first:   # Only one event is in the list
            self.first = None
            self.last = None
        else:   # Multiple events is in the list
